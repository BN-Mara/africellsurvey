            ...
            if (viewHolder != null && viewHolder.itemView != null) {
                if (viewHolder instanceof EditTextViewHolder) {
                    ...
                    for (int j = 0; j < ((CheckboxxViewHolder) viewHolder).chCheck.getChildCount(); j++) {
                        ((CheckBox) ((CheckboxxViewHolder) viewHolder).chCheck.getChildAt(j)).setError(null);
                    }
                }
            }
        }
            ...
            if (jsonModel.getRequired() != null && jsonModel.getRequired()) {
                if (viewHolder != null && viewHolder.itemView != null) {
                    if (jsonModel.getType() == FormConstants.TYPE_EDITTEXT
                        ...
                        isValidated[0] = false;
                    }else if (jsonModel.getType() == FormConstants.TYPE_CHECKBOXX
                            && fieldValue.equalsIgnoreCase(EMPTY_STRING)) {
                        for (int j = 0; j < ((CheckboxxViewHolder) viewHolder).chCheck.getChildCount(); j++) {
                            ((CheckBox) ((CheckboxxViewHolder) viewHolder).chCheck.getChildAt(j)).setError(FormConstants.FIELD_REQUIRED);
                        }
                        recyclerView.smoothScrollToPosition(i);
